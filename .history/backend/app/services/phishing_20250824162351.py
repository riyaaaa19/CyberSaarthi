...
class PhishingVerdict:
    verdict: str   # localized string now
    score: float
    explanation: str
...
class PhishingDetector:
    ...
    def _score_urls(self, text: str) -> Tuple[float, list[str]]:
        ...
        for u in urls:
            ...
            if d in self.blocklist:
                risk += 0.6  # higher weight for blocklisted domains
                reasons.append(f"Blocklisted domain: {d}")
            else:
                risk += 0.4  # slight suspicion for other domains
    ...
    def _score_text(self, text: str) -> Tuple[float, list[str]]:
        ...
        if count_terms > 0:
            reasons.append(f"Contains high-risk phishing terms ({count_terms} matches)")
            risk += 0.55  # increased weight for multiple matches
        if sum(1 for c in text if c.isupper()) / max(1, len(text)) > 0.2:
            reasons.append("Unusually high uppercase ratio")
            risk += 0.2  # increased weight for all-caps
        return min(risk, 1.0), reasons
    ...
    def _localized_verdict(self, verdict: Verdict, lang: str) -> str:
        if lang == "hi":
            if verdict == Verdict.SAFE:
                ...
                return "फ़िशिंग / असुरक्षित"
        return verdict.value

    def _localized_explanation(self, parts: list[str], lang: str) -> str:
        if lang != "hi":
            return "; ".join(parts)
        translated = []
        for p in parts:
            if p.startswith("ML score="):
                translated.append(p.replace("ML score=", "एमएल स्कोर="))
            elif "Blocklisted domain" in p:
                translated.append(p.replace("Blocklisted domain", "ब्लॉकलिस्टेड डोमेन"))
            elif "Suspicious domain pattern" in p:
                translated.append(p.replace("Suspicious domain pattern", "संदिग्ध डोमेन पैटर्न"))
            elif "Suspicious domain" in p:
                translated.append(p.replace("Suspicious domain", "संदिग्ध डोमेन"))
            elif "Punycode-like domain" in p:
                translated.append(p.replace("Punycode-like domain", "प्युनीकोड जैसा डोमेन"))
            elif "Contains high-risk phishing terms" in p:
                translated.append("संदेश में उच्च-जोखिम वाले फ़िशिंग शब्द हैं")
            elif "Unusually high uppercase ratio" in p:
                translated.append("असामान्य रूप से अधिक बड़े अक्षर अनुपात")
            else:
                translated.append(p)
    ...
    def predict(self, text: str) -> PhishingVerdict:
        ...
        explanation = self._localized_explanation(explanation_parts, "hi" if lang == "hi" else "en")
        localized_verdict = self._localized_verdict(verdict, "hi" if lang == "hi" else "en")

        return PhishingVerdict(
            verdict=localized_verdict,
            score=round(score, 3),
