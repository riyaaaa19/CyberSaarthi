...
from backend.app.utils.i18n import translate_explanation, msg
...
class PhishingDetector:
    ...
    def _score_text(self, text: str, lang: str) -> Tuple[float, list[str]]:
        ...
        matched_terms = [term for term in SUSPICIOUS_TERMS if term.lower() in lower_text]

        if matched_terms:
            terms_str = ", ".join(matched_terms[:5])
            if len(matched_terms) > 5:
                terms_str += f" ... (+{len(matched_terms)-5} more)"
            reasons.append(f"{msg(lang,'explanation.highRiskTerms')}: {terms_str}")
            risk += 0.6
    ...
    def predict(self, text: str, lang: str = "en") -> PhishingVerdict:
        ...
        explanation_parts = [f"{msg(lang,'explanation.mlScore')}: {ml_score:.2f}"] + url_reasons + txt_reasons
