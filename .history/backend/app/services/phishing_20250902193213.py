...
from app.utils.text import find_urls
from app.schemas import Verdict
from app.utils.i18n import translate_explanation, msg   # ✅ Import translations
...
class PhishingVerdict:
    verdict: Verdict
    score: float
    explanations: list[str]   # ✅ detailed list
    explanation: str          # ✅ single joined string
...
class PhishingDetector:
    ...
    def _score_urls(self, text: str, lang: str) -> Tuple[float, list[str]]:
        ...
        for u in urls:
            ...
            if d in self.blocklist:
                risk += 0.6
                reasons.append(f"{msg(lang,'explanation.suspiciousDomain')}: {d}")
            else:
                risk += 0.4
                reasons.append(f"{msg(lang,'explanation.suspiciousDomain')}: {d}")
    ...
    def predict(self, text: str, lang: str = "en") -> PhishingVerdict:
        ...
        explanations: list[str] = [f"{msg(lang,'explanation.mlScore')}: {ml_score:.2f}"] + url_reasons + txt_reasons

        if score >= 0.6:
            verdict = Verdict.MALICIOUS
            explanations.insert(0, msg(lang, "email.mal"))
        elif score >= 0.35:
            verdict = Verdict.SUSPICIOUS
            explanations.insert(0, msg(lang, "email.susp"))
        else:
            verdict = Verdict.SAFE
            explanations.insert(0, msg(lang, "email.safe"))
