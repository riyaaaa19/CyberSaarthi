...
from backend.app.utils.text import find_urls
from backend.app.schemas import Verdict
from backend.app.utils.i18n import translate_explanation, msg   # ✅ translations

logger = logging.getLogger(__name__)
settings = get_settings()

# Expanded phishing term list (English + Hindi)
SUSPICIOUS_TERMS = [
    ...
    # Hindi equivalents
    "खाता निलंबित", "तुरंत", "पासवर्ड", "अपडेट करें", "लॉगिन करें", "इनाम", "लॉटरी", "सत्यापित करें"
]
...
class PhishingVerdict:
    verdict: Verdict
    score: float
    explanations: list[str]   # 👈 structured explanations
...
class PhishingDetector:
    ...
    def _score_urls(self, text: str, lang: str) -> Tuple[float, list[str]]:
        ...
        for u in urls:
            ...
            if d in self.blocklist:
                risk += 0.6
                reasons.append(f"{msg(lang,'explanation.suspiciousDomainFound')}: {d}")
            else:
                risk += 0.4
                reasons.append(f"{msg(lang,'explanation.suspiciousDomainFound')}: {d}")

            if any(c.isdigit() for c in d) and any(c.isalpha() for c in d) and d.count('-') >= 2:
                risk += 0.25
                reasons.append(msg(lang, "explanation.suspiciousPattern") + f": {d}")

            if self._is_punycode(d):
                risk += 0.2
                reasons.append(msg(lang, "explanation.punycode") + f": {d}")
    ...
    def _score_text(self, text: str, lang: str) -> Tuple[float, list[str]]:
        ...
        count_terms = sum(1 for term in SUSPICIOUS_TERMS if term in lower_text)
        if count_terms > 0:
            reasons.append(f"{msg(lang,'explanation.highRiskTerms')} ({count_terms})")
            risk += 0.6

        if sum(1 for c in text if c.isupper()) / max(1, len(text)) > 0.2:
            reasons.append(msg(lang,"explanation.uppercase"))
            risk += 0.2
    ...
    def predict(self, text: str, lang: str = "en") -> PhishingVerdict:
        ...
        if score >= 0.6:
            ...
            explanations.append(msg(lang, "email.mal"))
        elif score >= 0.35:
            verdict = Verdict.SUSPICIOUS
            explanations.append(msg(lang, "email.susp"))
        else:
            verdict = Verdict.SAFE
            explanations.append(msg(lang, "email.safe"))

        explanations.append(f"{msg(lang,'explanation.mlScore')}: {ml_score:.2f}")
        explanations.extend(url_reasons)
        explanations.extend(txt_reasons)
