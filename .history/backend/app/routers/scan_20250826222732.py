...
def scan_email(payload: EmailScanRequest, settings: Settings = Depends(settings_dep), db: Session = Depends(db_dep)):
    ...
    record = ScanHistory(
        ...
        explanation="; ".join(result.explanations),  # store string
        lang=payload.lang,
    )
    db.add(record)
    db.commit()
    ...
    return ScanResponse(
        ...
        score=result.score,
        explanation="; ".join(result.explanations),  # ✅ single string
        explanations=result.explanations,            # ✅ structured list
        lang=payload.lang,
    )
