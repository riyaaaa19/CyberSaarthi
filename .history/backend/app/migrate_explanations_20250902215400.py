...
def run():
    ...
    for r in reports:
        ...
        if (
            isinstance(explanation_obj, dict)
            and explanation_obj.get("en")  # type: ignore
            and explanation_obj.get("hi")  # type: ignore
        ...
        if is_double_encoded(explanation_obj):
            try:
                hi_val: Optional[str] = explanation_obj["hi"] if (isinstance(explanation_obj, dict) and "hi" in explanation_obj) else None
        ...
        hindi_exp = ""
        if isinstance(explanation_obj, dict) and explanation_obj.get("hi"):  # type: ignore
            hindi_exp = explanation_obj.get("hi")  # type: ignore
        else:
