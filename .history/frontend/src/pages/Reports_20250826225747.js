...
function Reports() {
  ...
  // ✅ Explanation Translator (always language-friendly + bullets)
  const formatExplanation = (explanation, scanType, verdict) => {
    ...
    return list.map((e) => {
      // Email cases
      if (scanType === "email") {
        if (e.includes("Email appears safe") || e.includes("ईमेल सुरक्षित"))
          return t("explanation.safe");
        if (e.includes("Suspicious indicators") || e.includes("संदिग्ध संकेत"))
          return t("explanation.suspicious");
        if (e.includes("High-risk indicators") || e.includes("उच्च-जोखिम"))
          return t("explanation.malicious");
      }

      // Invoice cases
      if (scanType === "invoice" || scanType === "invoice_file") {
        if (e.includes("Invoice looks consistent") || e.includes("इनवॉइस सही"))
          return t("explanation.invoice.consistent");
        if (e.includes("Missing explicit total amount") || e.includes("कुल राशि"))
          return t("explanation.invoice.missingTotal");
        if (e.includes("Bank details without GSTIN") || e.includes("बैंक विवरण"))
          return t("explanation.invoice.bankDetails");
        if (e.includes("Account number present") || e.includes("खाता संख्या"))
      ...
      if (e.toLowerCase().includes("suspicious domain")) {
        const domain = e.split(":").pop().trim();
        return `${t("explanation.suspiciousDomain")}: ${domain}`;
      }
      if (e.toLowerCase().includes("high-risk phishing terms")) {
