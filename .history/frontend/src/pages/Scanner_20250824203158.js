  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      {result && (
        ...
        >
          <p>
            <b>{t("dashboard.verdict")}:</b>{" "}
            {t(`email.${result.verdict.toLowerCase()}`, result.verdict)}
          </p>
          <p><b>{t("dashboard.score")}:</b> {result.score ?? "N/A"}</p>
          <p><b>{t("dashboard.explanation")}:</b> {result.explanation}</p>
        </div>
      )}
    </div>
  );
