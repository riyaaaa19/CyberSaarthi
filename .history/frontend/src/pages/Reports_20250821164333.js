          ...
          {reports.length > 0 ? (
            reports.map((r, i) => (
              <tr key={i}>
                ...
                <td style={tdStyle}>{r.verdict === "safe" ? t("email.safe") : ""}</td>
                <td style={tdStyle}>{r.verdict === "susp" ? t("email.susp") : ""}</td>
                <td style={tdStyle}>{r.verdict === "mal" ? t("email.mal") : ""}</td>
                <td style={tdStyle}>{new Date(r.created_at).toLocaleString()}</td>
              </tr>
            ))
          ) : (
            <tr>
              <td style={tdStyle} colSpan="5">⚠️ No reports available</td>
            </tr>
          )}
        </tbody>
      </table>
