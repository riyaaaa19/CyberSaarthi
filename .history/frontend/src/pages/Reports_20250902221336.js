...
function Reports() {
  ...
  useEffect(() => {
    const onLangChange = () => setCurrentLang(localStorage.getItem("LANG") || "en");
  ...
  const formatExplanation = (explanation, scanType, verdict) => {
    ...
    // If explanation is an object, pick the current language and ensure it's a string
    let expText = explanation;
    if (typeof explanation === "object" && explanation !== null) {
      expText =
        typeof explanation[currentLang] === "string"
          ? explanation[currentLang]
