...
function Reports() {
  ...
  const formatExplanation = (explanation, scanType, verdict) => {
    ...
    return list.map((e) => {
      // Email cases
      if (scanType === "email") {
        if (e.includes("Email appears safe")) return t("explanation.safe");
        if (e.includes("Suspicious indicators"))
          return t("explanation.suspicious");
        if (e.includes("High-risk indicators"))
      ...
      // Technical
      if (e.includes("ML score")) {
        const val = e.split(":").pop();
        return `${t("explanation.mlScore")}: ${val}`;
      }
      if (e.includes("Suspicious domain")) {
        const domain = e.split(":").pop();
        return `${t("explanation.suspiciousDomain")}: ${domain}`;
      }
      if (e.includes("high-risk phishing terms")) {
