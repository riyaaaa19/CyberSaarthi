  ...
  const formatExplanation = (explanation, scanType, verdict) => {
    ...
    if (scanType === "email") {
      ...
      if (verdict === "SUSPICIOUS")
        explanation = explanation.replace(
          "Suspicious indicators found (possible phishing).",
          t("explanation.suspicious")
        );
      if (verdict === "MALICIOUS")
        explanation = explanation.replace(
          "High-risk indicators found (likely phishing).",
    ...
    if (scanType === "invoice" || scanType === "invoice_file") {
      explanation = explanation.replace(
        "Invoice looks consistent.",
        t("explanation.invoice.consistent")
      );
      explanation = explanation.replace(
        "Missing explicit total amount",
        t("explanation.invoice.missingTotal")
      );
      explanation = explanation.replace(
        "Bank details without GSTIN reference",
    ...
    // ✅ Technical part
    explanation = explanation.replace("ML score", t("explanation.mlScore"));
    explanation = explanation.replace(
      "Suspicious domain",
      t("explanation.suspiciousDomain")
    );
