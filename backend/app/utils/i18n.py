...
MESSAGES = {
    "en": {
        ...
        "explanation.highRiskTerms": "Contains high-risk phishing terms",
        "explanation.uppercase": "Unusually high uppercase ratio",
        "explanation.suspiciousPattern": "Suspicious domain pattern",
        "explanation.punycode": "Punycode-like domain",
    },
    "hi": {
        # Email explanations
        ...
        "explanation.highRiskTerms": "उच्च-जोखिम वाले फ़िशिंग शब्द शामिल हैं",
        "explanation.uppercase": "असामान्य रूप से अधिक अपरकेस अनुपात",
        "explanation.suspiciousPattern": "संदिग्ध डोमेन पैटर्न",
        "explanation.punycode": "प्युनिकोड जैसा डोमेन",
    },
}
...
def translate_explanation(lang: str, explanation: str) -> str:
    """
    Replaces English explanation fragments with localized equivalents,
    keeps domains, numbers and counts unchanged.
    """
    # Replace known fragments
    explanation = ci_replace(explanation, r"ML score", msg(lang, "explanation.mlScore"))
    explanation = ci_replace(explanation, r"Suspicious domain found", msg(lang, "explanation.suspiciousDomainFound"))
    explanation = ci_replace(explanation, r"Suspicious domain pattern", msg(lang, "explanation.suspiciousPattern"))
    explanation = ci_replace(explanation, r"Punycode-like domain", msg(lang, "explanation.punycode"))
    explanation = ci_replace(explanation, r"Contains high-risk phishing terms", msg(lang, "explanation.highRiskTerms"))
    explanation = ci_replace(explanation, r"Unusually high uppercase ratio", msg(lang, "explanation.uppercase"))
    explanation = ci_replace(explanation, r"Suspicious domain", msg(lang, "explanation.suspiciousDomain"))

    # Make separators user-friendly
    if lang == "hi":
